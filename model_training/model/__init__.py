...
class Model(nn.Module):
    def __init__(self, args, ckp):
        ...
        self.cpu = args.cpu
        self.device = torch.device("cuda:{}".format(args.id_GPU) if torch.cuda.is_available() else "cpu")
        # self.device = torch.device("cuda:{}".format(args.id_GPU) if torch.cuda.is_available() else "cpu")
    ...
    def forward(self, x, idx_scale):
        ...
        if self.training:
            if self.n_GPUs > 1:
                return P.data_parallel(self.model, x , device_ids=range(self.id_GPU, self.id_GPU + self.n_GPUs))
            else:
                return self.model(x)
        else:
            if self.chop:
    ...
    def forward_chop(self, *args, shave=10, min_size=160000):
        ...
        if h * w < 4 * min_size:
            for i in range(0, 4, n_GPUs):
                ...
                y = P.data_parallel(self.model, *x, range(self.id_GPU, self.id_GPU + n_GPUs))
