...
class Trainer():
    ...
    def train(self):
        ...
        for batch, (data_pack, _) in enumerate(self.loader_train):
            ...
            loss = self.loss(estimation, ve, pm) + self.loss(estimation, te, pm) + self.loss(0.5 * enh[:, 3, :, :].unsqueeze(1), ve_thick, pm) + self.pc(enh, _, pm)# + 0.01*self.tv_loss(enh)# + self.pbce_loss(estimation, ve, pm*ma)  #l1 supervisions
            loss.backward()
        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):
                ...
                for data_pack, filename in tqdm(d, ncols=80):
                    ...
                    enhance = enhance[:, 0:3, 0:960, 0:999]
                    ve = ve[:, :, 0:960, 0:999]
                    hr = hr[:, :, 0:960, 0:999]
