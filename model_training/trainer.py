...
import data
# from sgl_training.loss import Loss
import utility
import numpy as np
import torch
...
from loss.bceloss import path_continuity_loss as PCL
from loss.bceloss import profile_consistency as PC
...
class Trainer():
    ...
    def train(self):
        ...
        for batch, (data_pack, _) in enumerate(self.loader_train):
            ...
            enh, estimation= self.model(hr, 1)  # enh torch.Size([16, 6, 256, 256])
            loss = self.loss(estimation, ve, pm*ma) + self.loss(estimation, te, pm*ma) + self.loss(0.5 * enh[:, 3, :, :].unsqueeze(1), ve_thick, pm*ma) + self.pc(enh, _, pm*ma)
            loss.backward()
        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):
                ...
                for data_pack, filename in tqdm(d, ncols=80):
                    ...
                    enhance = enhance[:, 0:3 , 0:584, 0:565]
                    ve = ve[:, :, 0:584, 0:565]
                    hr = hr[:, :, 0:584, 0:565]
    ...
    def prepare(self, args):  # the data to GPU 
        # device = torch.device('cpu' if self.args.cpu else 'cuda')
        device = torch.device("cuda:{}".format(self.id_GPU) if torch.cuda.is_available() else "cpu")
        def _prepare(tensor):
