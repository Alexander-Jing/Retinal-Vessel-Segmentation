    ...
    def __init__(self, args, ckp):
        ...
        for loss in args.loss.split('+'):
            ...
            if loss_type == 'MSE':
                ...
                #loss_function = nn.BCELoss()
                module = import_module('loss.bceloss')
                loss_function = getattr(module, 'path_continuity_loss')()  # profile_consistency
            elif loss_type == 'PC':
                #loss_function = nn.BCELoss()
                module = import_module('loss.bceloss')
                loss_function = getattr(module, 'profile_consistency')()
        ...
        device = torch.device("cuda:{}".format(args.id_GPU) if torch.cuda.is_available() else "cpu")
        self.loss_module.to(device)
        if args.precision == 'half': self.loss_module.half()
        if not args.cpu and args.n_GPUs > 1:
            self.loss_module = nn.DataParallel(
                self.loss_module, range(args.id_GPU, args.id_GPU + args.n_GPUs)
            )
