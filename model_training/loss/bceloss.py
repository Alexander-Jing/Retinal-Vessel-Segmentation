...
class profile_consistency(nn.Module):
    def __init__(self):
        super(profile_consistency, self).__init__()
    def forward(self, y_pred, y_true, pmap):
        B, C, W, H = y_pred.size()
        #pc = - y_true*torch.log(y_pred+1e-14) - (1 - y_true) * torch.log(1 - y_pred + 1e-14)
        pc = torch.sum(y_pred[:,4:6,0:255,:]*y_pred[:,4:6,1:256,:]) + torch.sum(y_pred[:,4:6,:,0:255]*y_pred[:,4:6,:,1:256])
        #pc = pc * pmap
        pc = (-1) *pc / (B*C*W*H)
        return pc

class path_continuity_loss(nn.Module):
    def __init__(self):
        super(path_continuity_loss, self).__init__()
    def forward(self, y_pred, y_true, dp):
        print("pcl loss")
        B, C, W, H = y_pred.size()
        pcl = torch.tensor([0.0]).to(torch.device("cuda:{}".format(0) if torch.cuda.is_available() else "cpu"))
        p = 0
        #for p in range(B): 
        for i in range(W):
            for j in range(H):
                x,y = i,j
                for _ in range(int(y_pred[p,3,i,j])+1):
                    x = int(x + y_pred[p,4,x,y])
                    y = int(y + y_pred[p,5,x,y])
                    pcl += torch.sum(y_pred[p,4:6,i,j]*y_pred[p,4:6,x,y])
        print("pcl")
        print(pcl)
        return (-1)*pcl/(B*W*H)
